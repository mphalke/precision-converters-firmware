...
projectName = "adt7420_console-application"

// This variable holds the build status
buildStatusInfo = "Success"

def adt7420ConsoleBuild() {
	Map buildMap =[:]
					...
					withEnv(axisEnv) {
						try {
							stage("Build-${PLATFORM_NAME}-${EVB_INTERFACE}") {
								...
								bat "cd ${projectName} & make test TARGET_BOARD=${PLATFORM_NAME} ARTIFACT-NAME=${PLATFORM_NAME}-${EVB_INTERFACE} TEST_FLAGS=-DPLATFORM_NAME=\\\\\\\"${PLATFORM_NAME}\\\\\\\" TEST_FLAGS+=-D${EVB_INTERFACE}"
								...
								stash includes: "${projectName}/build/**/*.bin, ${projectName}/build/**/*.elf", name: "${PLATFORM_NAME}-${EVB_INTERFACE}"
								deleteDir()
							}
						}
						catch (Exception ex) {
...
def runSeqBuild(Map config =[:]) {
	stage("Build-${config.PLATFORM_NAME}-${config.EVB_INTERFACE}") {
		...
		echo "Starting mbed build..."
		//NOTE: if adding in --profile, need to change the path where the .bin is found by mbedflsh in Test stage
		bat "cd ${projectName} & make test TARGET_BOARD=${config.PLATFORM_NAME} ARTIFACT-NAME=${config.PLATFORM_NAME}-${config.EVB_INTERFACE} TEST_FLAGS=-DPLATFORM_NAME=\\\\\\\"${config.PLATFORM_NAME}\\\\\\\" TEST_FLAGS+=-D${config.EVB_INTERFACE}"
		...
		archiveArtifacts allowEmptyArchive: true, artifacts: "${projectName}/build/**/*.bin, ${projectName}/build/**/*.elf"
		stash includes: "${projectName}/build/**/*.bin, ${projectName}/build/**/*.elf", name: "${config.PLATFORM_NAME}-${config.EVB_INTERFACE}"
		bat "cd ${projectName} & make clean TARGET_BOARD=${config.PLATFORM_NAME}"
	}
}

def adt7420ConsoleTest() {
	Map testMap =[:]
	if (env.CHANGE_TARGET == "main" || env.CHANGE_TARGET == "develop") {
		...
		ws('workspace/pcg-fw') {
			...
			try {
				stage("Test-${config.PLATFORM_NAME}-${config.EVB_INTERFACE}") {
					...
					bat returnStatus: true , script: "mbedflsh --disk ${platformInfo["mountpoint"]} --file ${WORKSPACE}\\${projectName}\\build\\${config.PLATFORM_NAME}\\${TOOLCHAIN}\\${config.PLATFORM_NAME}-${config.EVB_INTERFACE}.bin"               
