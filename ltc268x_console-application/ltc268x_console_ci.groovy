...
projectName = "ltc268x_console-application"
					...
					withEnv(axisEnv) {
						try {
							stage("Build-${PLATFORM_NAME}-${ACTIVE_DEVICE}") {
								...
								bat "cd ${projectName} & make test TARGET_BOARD=${PLATFORM_NAME} ARTIFACT-NAME=${PLATFORM_NAME}-${ACTIVE_DEVICE} TEST_FLAGS=-DPLATFORM_NAME=\\\\\\\"${PLATFORM_NAME}\\\\\\\" TEST_FLAGS+=-D${ACTIVE_DEVICE}"
								archiveArtifacts allowEmptyArchive: true, artifacts: "${projectName}/build/**/*.bin, ${projectName}/build/**/*.elf"
								stash includes: "${projectName}/build/**/*.bin, ${projectName}/build/**/*.elf", name: "${PLATFORM_NAME}-${ACTIVE_DEVICE}"
								deleteDir()
							}
						}
						catch (Exception ex) {
...
def runSeqBuild(Map config =[:]) {
	try {
		stage("Build-${config.PLATFORM_NAME}-${config.ACTIVE_DEVICE}") {
			...
			echo "Starting mbed build..."
			//NOTE: if adding in --profile, need to change the path where the .bin is found by mbedflsh in Test stage
			bat "cd ${projectName} & make test TARGET_BOARD=${config.PLATFORM_NAME} ARTIFACT-NAME=${config.PLATFORM_NAME}-${config.ACTIVE_DEVICE} TEST_FLAGS=-DDEVICE_NAME=\\\\\\\"${config.ACTIVE_DEVICE}\\\\\\\" TEST_FLAGS+=-DPLATFORM_NAME=\\\\\\\"${config.PLATFORM_NAME}\\\\\\\" TEST_FLAGS+=-D${config.ACTIVE_DEVICE}"
			archiveArtifacts allowEmptyArchive: true, artifacts: "${projectName}/build/**/*.bin, ${projectName}/build/**/*.elf"
			stash includes: "${projectName}/build/**/*.bin, ${projectName}/build/**/*.elf", name: "${config.PLATFORM_NAME}-${config.ACTIVE_DEVICE}"
			bat "cd ${projectName} & make clean TARGET_BOARD=${config.PLATFORM_NAME}"
		}
	}
	catch (Exception ex) {
			echo "Failed in Build-${config.PLATFORM_NAME}-${config.ACTIVE_DEVICE} stage"
			echo "Caught:${ex}"
			buildStatusInfo = "Failed"
			currentBuild.result = 'FAILURE'
			bat "cd ${projectName} & make clean TARGET_BOARD=${config.PLATFORM_NAME}"
